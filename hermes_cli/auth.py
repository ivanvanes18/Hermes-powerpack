...
def _select_auth_store_owner_ids(
    ...
    """Choose the POSIX owner to preserve across a root-authored write."""
...
def _file_lock(
    ...
    try:
        if stat.S_ISLNK(lock_path.lstat().st_mode):
            raise OSError(f"Refusing symlink lock path: {lock_path}")
    except FileNotFoundError:
        pass
    ...
    if msvcrt:
        lock_context = lock_path.open("r+", encoding="utf-8")
    else:
        flags = os.O_RDWR | os.O_CREAT | os.O_APPEND
        if hasattr(os, "O_NOFOLLOW"):
            flags |= os.O_NOFOLLOW
        fd = os.open(str(lock_path), flags, stat.S_IRUSR | stat.S_IWUSR)
        lock_context = os.fdopen(fd, "a+", encoding="utf-8")
