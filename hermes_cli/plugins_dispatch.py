...
class PluginDispatchMixin:
    ...
    def _run_hook_callback_bounded(
        ...
        with self._hook_timeout_lock:
            ...
            # Ordinary post-tool observer events may overlap when they have distinct identities;
            # a timed-out/abandoned worker still blocks the callback to prevent thread growth.
            allow_overlap = (hook_name == "post_tool_call"
                             and gate_key not in self._hook_running_callbacks
                             and not self._hook_abandoned.get(suppression_key))
