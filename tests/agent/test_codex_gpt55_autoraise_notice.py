...
def test_gateway_notice_uses_replay_once_without_startup_print(monkeypatch, tmp_path):
    ...
    delivered = []
    agent.status_callback = lambda event, message: delivered.append((event, message))
    agent._replay_compression_warning()
    agent._compression_warning = None
    agent._replay_compression_warning()

    assert len(delivered) == 1
    assert "auto-compaction was raised" in delivered[0][1]
