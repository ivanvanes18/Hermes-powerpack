...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _gptprof_profile_dir(self) -> _Path:
        ...
        return _Path(
            os.getenv("HERMES_HCP", str(base / "gptprof" / "profiles"))
        ).expanduser()
    ...
    def _gptprof_switch_profile(self, slug: str, model: str) -> tuple[bool, str]:
        ...
        if is_managed():
            return (
                False,
                "This Hermes installation is managed; gptprof cannot persist its model route.",
            )
        ...
        try:
            ...
            with auth_mod._file_lock(
                ...
                for item in old_pool:
                    ...
                    if (
                        item.get("id") == entry_id
                        or item_source == entry_id
                        or item_profile == slug
                    ):
                        continue
                ...
                persisted_model = (
                    persisted.get("model") if isinstance(persisted, dict) else None
                )
                if not (
        ...
        return (
            True,
            f"Profile `{slug}` activated with `{model}`. Send `/new` for a fresh session.",
        )
    ...
    async def _handle_gptprof_callback(self, query, data: str) -> None:
        ...
        if action in {"refresh", "card"}:
            ...
            command = [
                sys.executable,
                os.path.expanduser("~/.local/bin/gptprof_send_buttons.py"),
            ]
        ...
        if action == "autoswitch":
            ...
            command = [
                sys.executable,
                os.path.expanduser("~/.local/bin/gptprof_autoswitch.py"),
            ]
            await self._run_gptprof_helper(
                query,
                command,
                "Autoswitch: no output",
                timeout=30,
            )
            return
        if len(parts) < 3:
        ...
        try:
            ...
            pass
        await query.answer(
            text="Profile switched" if ok else "Switch failed",
            show_alert=not ok,
        )
