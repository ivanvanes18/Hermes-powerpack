...
def switch_profile_auth(slug: str, profile: dict[str, Any]) -> None:
    ...
    if profile.get("_pool_entry_generic"):
        entry_id = str(profile.get("_pool_entry_id") or "").strip()
        if not entry_id:
            raise ValueError("Generic profile has no credential pool id")
        from hermes_cli import auth as auth_mod

        auth_path = Path(gptprof.AUTH_PATH).expanduser()
        with auth_mod._auth_store_lock(target_path=auth_path):
            ...
            matches = [
                item for item in pool_list
                if isinstance(item, dict)
                and item.get("source") == "manual:device_code"
                and item.get("id") == entry_id
            ]
            if len(matches) != 1:
                raise ValueError(f"Credential pool entry {entry_id!r} is missing or ambiguous")
            selected = matches[0]
            if not selected.get("access_token") or not selected.get("refresh_token"):
                raise ValueError(f"Credential pool entry {entry_id!r} is incomplete")
...
async def collect_usage(profiles: dict[str, dict[str, Any]], catalog: list[tuple[str, str, str]]) -> dict[str, dict[str, Any]]:
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for slug, _plan, _model in catalog:
            ...
            if access_value:
                tasks.append(gptprof.fetch_usage(session, access_value, slug, cache))
