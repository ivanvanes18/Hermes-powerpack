...
[Service]
Type=oneshot
WorkingDirectory=/path/to/hermes/skills/gptprof-hermes
...
systemctl --user enable --now gptprof-token-refresh.timer
systemctl --user start gptprof-token-refresh.service
```
...
```bash
/path/to/hermes/.venv/bin/python ~/.local/bin/gptprof_autoswitch.py
```
