...
class CredentialPool(CredentialPoolAdminMixin, CredentialPoolModelCooldownMixin):
    ...
    def select(self, preferred_credential_id: Optional[str] = None, *, model: Optional[str] = None) -> Optional[PooledCredential]:
        ...
        *preferred_credential_id* is an affinity, not a guarantee: the pinned
        ...
        """
        entry, pending_refresh = self._select_under_lock(preferred_credential_id, model=model)
        if pending_refresh:
            ...
            if entry is None or (
                ...
                entry, _ = self._select_under_lock(
                    preferred_credential_id, count_pending_fallback=True, model=model,
                )
    ...
    def _select_under_lock(
        ...
        *, count_pending_fallback: bool = False, model: Optional[str] = None,
    ) -> Tuple[Optional[PooledCredential], List[PooledCredential]]:
        with self._lock:
            return self._select_unlocked(
                preferred_credential_id=preferred_credential_id,
                count_pending_fallback=count_pending_fallback, model=model,
            )
    ...
    def _available_entries(
        ...
        excluded_credential_ids: Optional[Set[str]] = None,
        ...
        *refresh* refreshes entries needing a token refresh (skipped on
        ...
        sole_credential = self._is_sole_credential()
        for entry in self._entries:
            # Immediate rotation can explicitly exclude the credential that just
            # failed. Skip it before any reset-marker, auth-store or quota-probe
            # reconciliation so selection cannot clear the failure we just wrote.
            if excluded_credential_ids and entry.id in excluded_credential_ids:
                continue
        ...
        *,
        ...
        with self._lock:
            ...
            if identity_supplied and failed_runtime_key:
                ...
                failed_entry_ids.update(s.id for s in siblings)
            ...
            self._current_id = None
            # Never allow the immediate recovery selection to re-adopt a row we just
            # benched. Cross-process token/reset reconciliation may legitimately make
            # that row look available again during selection; choosing it would both
            # retry the same failed credential and let round-robin persistence erase
            # the failure status before the next topic turn.
            next_entry, _pending = self._select_unlocked(
                refresh=False, excluded_credential_ids=failed_entry_ids,
            )
            if next_entry:
