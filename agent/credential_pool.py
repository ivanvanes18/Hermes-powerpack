...
class CredentialPool:
    ...
    def _sync_manual_entry_from_pool_store(
        ...
        """Re-read a manual entry while holding the shared auth lock.

        Manual OAuth refresh tokens are single-use. A process that waits for
        another refresh must adopt the rotated on-disk pair instead of
        replaying its stale in-memory token.
        """
        ...
        try:
            persisted = next(
                (
                    item
                    for item in read_credential_pool(self.provider)
