...
def build_memory_guidance(memory_enabled: bool = True, profile_enabled: bool = True) -> str:
    ...
    return frame + (
        ...
        "For read-only, audit, or staging-only work, record a proposed change in the requested artifact, "
        "not in installed skills or memory. Changes to permission, scope, or workflow-routing rules "
        "require explicit approval for those rules; routine learning within authorized work stays available. "
...
SKILLS_GUIDANCE = (
    "When authorized to update procedural knowledge, record a non-trivial workflow with skill_manage. "
    "In read-only, audit, or staging-only work, include the proposed lesson in the deliverable instead.\n\n"
    "## Skill Safety Rule\n"
...
def _render_skills_index(
    ...
    return (
        ...
        "If a skill has issues, distinguish a proposed correction from an authorized installed update. "
        "In read-only, audit, or staging-only work, put corrections in the requested deliverable; "
        "do not modify installed skills or memory. Outside those modes, save reusable lessons within "
        "the authorized scope. Obtain explicit approval for changes to permission, scope, or "
        "workflow-routing rules. Skill maintenance must not expand or delay the user's task.\n"
        "\n"
        "<available_skills>\n"
        + "\n".join(index_lines) + "\n"
        "</available_skills>\n\n"
