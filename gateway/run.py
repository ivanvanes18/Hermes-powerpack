...
def _load_gateway_runtime_config(config_path=None):
    """Compatibility name retained for status helpers and plugin/test callers."""
...
def _resolve_runtime_agent_kwargs(preferred_credential_id: Optional[str] = None) -> dict:
    ...
    own providers and never inherits the pin. An unpinned AuthError walks the configured fallback
    chain through the shared resolver."""
...
def _resolve_runtime_agent_kwargs_for_provider(
        provider: str, *, preferred_credential_id: Optional[str] = None,
        target_model: Optional[str] = None) -> dict:
    """Resolve runtime credentials for a specific provider, preserving topic affinity and model routing."""
    from hermes_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error
    try:
        runtime = resolve_runtime_provider(
            requested=provider, target_model=target_model or None,
...
def _format_status_count(value: Any) -> str:
    """Format compact counts for gateway status (1.4k, 2.1m)."""
    try:
...
def _format_status_duration(seconds: Any) -> str:
    """Human compact duration for /status uptime fields."""
    try:
...
def _status_git_revision() -> str:
    """Best-effort short git SHA for the active checkout; never blocks status."""
    try:
        import subprocess
...
def _gateway_status_model_parts(config: dict) -> tuple[str, str, str, Optional[int]]:
    """Return (model, provider, base_url, configured context length)."""
...
class GatewayRunner(
    ...
    def _resolve_profile_home_for_source(self, source: SessionSource) -> "Path":
        ...
        try:
            ...
            if explicit_profile and not profile_exists(name) and not profile_dir.is_dir():
                logger.warning(
    ...
    class _RunAgentDisplay:
        ...
        terminal_progress_format: Any = None
