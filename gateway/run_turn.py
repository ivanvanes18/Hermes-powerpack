...
class GatewayTurnMixin:
    ...
    def _resolve_session_agent_runtime(
        ...
        if ch:
            ...
            if ch.provider:
                runtime_kwargs = _resolve_runtime_agent_kwargs_for_provider(
                    ch.provider, preferred_credential_id=preferred_credential_id, target_model=model or None)
    ...
    def _run_agent_display_settings(self, source: SessionSource) -> "GatewayRunner._RunAgentDisplay":
        ...
        terminal_progress_format = str(
            resolve_display_setting(user_config, platform_key, "terminal_progress_format", "code_block")
            or "code_block"
        ).strip().lower()
    ...
    _DISPLAY_TO_TURN_CTX = (
        ...
        "progress_grouping", "terminal_progress_format", "tool_progress_enabled", "log_queue", "resolve_display_setting",
    ...
    async def _run_agent_mark_streamed_delivery(self, response: Any, turn_ctx: TurnContext) -> None:
        ...
        if not _transformed and (_streamed or _content_delivered):
            ...
            # never fed the final's deltas (interim-only wiring, #105341).
            logger.warning(
