...
class GatewaySlashCommandsMixin:
    ...
    async def _persist_model_switch_runtime(self, session_id: str, result: Any) -> None:
        ...
        try:
            existing = await self._session_db.get_session(session_id) or {}
        ...
        try:
            if hasattr(self._session_db, "update_session_meta"):
                await self._session_db.update_session_meta(
                    session_id,
                    json.dumps(model_config, ensure_ascii=False),
                    result.new_model,
                )
            else:
                await self._session_db.update_session_model(session_id, result.new_model)
